...
const ApiService = {
  ...
  certification: {
    ...
    update: async (certificationId, updateData) => {
      try {
        const response = await api.put(`/certifications/${certificationId}`, updateData);
        return response.data;
      } catch (error) {
  ...
  topic: {
    ...
    getByCertification: async (certificationId) => {
      try {
        const response = await api.get(`/topics/certification/${certificationId}`);
        return response.data;
      } catch (error) {
    ...
    create: async (subtopicData) => {
      try {
        ...
        if (processedData.title && processedData.title.length > 100) {
          console.warn(`Subtopic title too long (${processedData.title.length} chars). Truncating to 100 characters.`);
          ...
          try {
            ...
            if (errorData && errorData.message) {
              ...
              if (typeof errorData.message === 'string' && 
                ...
                throw new Error(errorData.message.split('validation failed:')[1]?.trim() || 
