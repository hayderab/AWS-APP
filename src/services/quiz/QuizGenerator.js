...
class QuizGenerator {
  constructor() {
    try {
      ...
      console.log(
        "QuizGenerator initialized with fallback model. Checking available models..."
      );
    ...
    try {
      ...
      try {
        ...
        const result = await chatSession.sendMessage(
          `Generate the ${numberOfQuestions} AWS questions now as a valid JSON array. Remember to return ONLY the JSON array with no additional text, markdown formatting, or code blocks. Start with '[' and end with ']'. Ensure all JSON properties are properly quoted.`
        );
        ...
        try {
          ...
          try {
            ...
            console.log(
              "Direct parsing failed, trying to extract JSON from response..."
            );
          ...
          try {
            ...
            if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
              ...
              try {
                ...
                console.log(
                  "Fixed JSON candidate:",
                  fixedCandidate.substring(0, 100) + "..."
                );
                ...
                try {
                  ...
                  const questionMatches = cleanedText.match(
                    /\{\s*"question_type"[^{]*?(?=\}\s*,|\}\s*\])/g
                  );

                  if (questionMatches && questionMatches.length > 0) {
                    console.log(
                      `Found ${questionMatches.length} individual questions to reconstruct`
                    );

                    // Add closing braces to each question
                    const completeQuestions = questionMatches.map(
                      (q) => q + "}"
                    );
                    ...
                    try {
                      ...
                      console.error(
                        "Reconstructed JSON parsing failed:",
                        reconstructError
                      );

                      // Last resort: create a minimal valid question set
                      console.log(
                        "Creating minimal valid question set as fallback"
                      );
                      questions = this.createFallbackQuestions(
                        numberOfQuestions,
                        topic,
                        subtopic
                      );
              ...
              // Last resort: create a minimal valid question set
              console.log(
                "JSON array markers not found. Creating minimal valid question set as fallback"
              );
              questions = this.createFallbackQuestions(
                numberOfQuestions,
                topic,
            ...
            // Last resort: create a minimal valid question set
            console.log(
              "Creating minimal valid question set as final fallback"
            );
            questions = this.createFallbackQuestions(
              numberOfQuestions,
              topic,
        ...
        if (!Array.isArray(questions)) {
          throw new Error(
            `Parsed result is not an array. Type: ${typeof questions}`
          );
        }
        ...
        if (questions.length < 1) {
          console.warn(
            `Expected at least one question but got ${questions.length}.`
          );
        ...
        const questionObjects = questions.map((q) =>
          this.createQuestionFromJSONFormat(q)
        );

        // Convert questions to backend format
        const backendFormattedQuestions = questions.map((q) => {
          let formattedQuestion = {
            question: q.question_text || q.question || "No question provided",
          ...
          if (
            ...
            q.question_type === "Multiple Response"
          ) {
            if (Array.isArray(q.options)) {
              ...
              if (q.correct_answer) {
                ...
                // For multiple response, use the first correct answer as the index
                const firstCorrect = q.correct_answers[0];
                const correctIndex = q.options.findIndex(
                  (opt) =>
                    (typeof opt === "object" ? opt.id : opt) === firstCorrect
                );
                formattedQuestion.correctAnswer =
                  correctIndex >= 0 ? correctIndex : 0;
              } else {
            ...
            // For other question types, create a simplified format
            formattedQuestion.options = [
              "See question details",
              "Interactive question type",
            ];
            formattedQuestion.correctAnswer = 0;
          }
        ...
        const apiRequestError = new Error(
      ...
      console.log(
        "Creating fallback questions for",
        topic?.title || "Unknown Topic",
        subtopic?.title || "Unknown Subtopic"
      );
  ...
  createFallbackQuestions(count, topic, subtopic) {
    console.log(
      `Creating ${count} fallback questions for ${
        topic?.title || "Unknown"
      } - ${subtopic?.title || "Unknown"}`
    );
    ...
    const createBasicQuestion = (index) => {
      return {
        ...
        question_text: `Question ${
          index + 1
        }: This is a fallback question about ${
          subtopic?.title || "the topic"
        }. The actual question generation failed.`,
        options: [
  ...
  createQuestionFromJSONFormat(questionData) {
    try {
      ...
      if (!questionData || !questionData.question_type) {
        console.error(
          "Invalid question data received for mapping:",
          questionData
        );
        throw new Error(
          "Invalid question data format passed to createQuestionFromJSONFormat"
        );
      }
      ...
      switch (questionData.question_type) {
        ...
        default:
          console.error(
            `Unknown question type encountered: ${questionData.question_type}`,
            questionData
          );
          throw new Error(
            `Unknown question type: ${questionData.question_type}`
          );
      }
      ...
      console.error(
        "Error creating question from specific JSON format:",
        error,
        "Data:",
      ...
      switch (questionData.question_type) {
        case "Multiple Choice":
          if (!questionData.options || !questionData.correct_answer)
            throw new Error("Missing options/correct_answer");
          const mcCorrectOption = questionData.options.find(
            (o) => o.id === questionData.correct_answer
          );
          if (!mcCorrectOption)
            throw new Error(
              `Correct answer ID '${questionData.correct_answer}' not found in options.`
            );
          return {
        ...
        case "Ordering":
          if (
            !questionData.items_to_order ||
            !questionData.correct_order ||
            !Array.isArray(questionData.correct_order)
          )
            throw new Error("Missing items_to_order/correct_order");
          const orderCorrectTexts = questionData.correct_order.map((id) => {
            const item = questionData.items_to_order.find((i) => i.id === id);
            if (!item)
              throw new Error(`Correct order ID '${id}' not found in items.`);
            return item.text;
          });
          return {
            question: questionData.question_text || "Missing question text",
        ...
        case "Matching":
          if (
            ...
            !Array.isArray(questionData.correct_matches)
          )
            throw new Error("Missing prompts/responses/correct_matches");
          const correctPairsMapped = questionData.correct_matches.map(
            (match) => {
              const prompt = questionData.prompts.find(
                (p) => p.id === match.prompt_id
              );
              const response = questionData.responses.find(
                (r) => r.id === match.response_id
              );
              if (!prompt || !response)
                throw new Error(
                  `Invalid prompt/response ID in match: ${JSON.stringify(
                    match
                  )}`
                );
        ...
        case "Case Study":
          if (
            ...
            throw new Error("Missing scenario/questions");
          ...
          const mappedSubQuestions = questionData.questions.map((sq, index) => {
            if (!sq.options || !sq.correct_answer) {
              throw new Error(
                `Sub-question ${index + 1} missing options/correct_answer`
              );
            }

            const sqCorrectOption = sq.options.find(
              (o) => o.id === sq.correct_answer
            );
            if (!sqCorrectOption) {
              throw new Error(
                `Sub-question ${index + 1} correct answer ID '${
                  sq.correct_answer
                }' not found.`
              );
            }
          ...
          return {
            question:
              (questionData.scenario || "Scenario").substring(0, 70) + "...", // Use first chars of scenario as title
            scenario: questionData.scenario || "Missing scenario text",
            subQuestions: mappedSubQuestions,
            explanation:
              questionData.explanation || "No overall explanation provided.",
            difficulty: "hard",
          };
        default:
          // This should have been caught earlier, but as a safeguard:
          throw new Error(
            `Unhandled question type in mapQuestionData: ${questionData.question_type}`
          );
      }
    } catch (mappingError) {
      console.error(
        "Error mapping data for type:",
  ...
  createQuestionFromJSON(questionData) {
    try {
      ...
      if (!questionData || !questionData.type) {
        console.error(
          "Invalid mapped data passed to createQuestionFromJSON:",
          questionData
        );
      ...
      if (!QuestionClass) {
        throw new Error(
          `Unknown question type for class instantiation: ${questionData.type}`
        );
      }
      ...
      console.error(
        "Error creating question instance from mapped JSON:",
        error,
        "Data:",
        JSON.stringify(questionData)
      );
  ...
  getConstructorArgs(questionData) {
    ...
    try {
      switch (questionData.type) {
        ...
        default:
          throw new Error(
            `Unknown question type in getConstructorArgs: ${questionData.type}`
          );
      }
    } catch (argError) {
      console.error(
        "Error getting constructor args for type:",
        questionData.type,
        argError
      );
